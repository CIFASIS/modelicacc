...
bool Connect::operator==(const Connect& other) const { return (other.left() == left() && other.right() == right()); }
...
bool CallEq::operator==(const CallEq& other) const { return (other.name() == name() && other.args() == args()); }
...
std::ostream& operator<<(std::ostream& out, const EquationSection& es)
{
  if (es.equations().size()) {
    if (es.initial()) out << "initial ";
    out << "equation\n";
  }
  foreach_(Equation e, es.equations()) out << INDENT << e << ";\n";
  return out;
}
}  // namespace AST
}  // namespace Modelica
