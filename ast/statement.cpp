...
std::ostream& operator<<(std::ostream& out, const StatementSection& ss)
{
  if (ss.statements().size()) {
    if (ss.initial()) out << "initial ";
    out << "algorithm\n";
  }
  BEGIN_BLOCK;
  foreach_(Statement s, ss.statements()) out << INDENT << s << ";\n";
  END_BLOCK;
  return out;
}
std::ostream& operator<<(std::ostream& out, const Assign& a)  // output
{
  if (a.right())
    ...
    foreach_(Expression e, el) out << e << (++i < size ? "," : "");
    out << ")";
  } else
    out << a.left();
  return out;
...
std::ostream& operator<<(std::ostream& out, const WhileSt& st)  // output
{
  ...
  BEGIN_BLOCK;
  foreach_(Statement s, st.elements()) { out << INDENT << s << ";\n"; }
  END_BLOCK;
