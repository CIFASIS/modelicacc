...
const char* ClassPrefixName[] = {"partial ",  "class ",      "model ", "block ",  "record ",  "connector ", "type ",
                                 "operator ", "expandable ", "pure ",  "impure ", "package ", "function "};

const char* classPrefix(Option<ClassPrefix> cp)
{
  if (cp) return ClassPrefixName[cp.get()];
  return "";
};
const char* typePrefix(Option<TypePrefix> tp)
{
  if (tp) return TypePrefixName[tp.get()];
  return "";
};
...
std::ostream& operator<<(std::ostream& out, const Class& c)  // output
{
  ...
  out << "\n";
  BEGIN_BLOCK;
  foreach_(Element e, c.composition().elements()) out << INDENT << e << ";\n";
  END_BLOCK;
  BEGIN_BLOCK;
  foreach_(CompElement ce, c.composition().comp_elem()) out << INDENT << ce;
  END_BLOCK;
  if (c.composition().annotation()) out << c.composition().annotation().get() << ";\n";
...
std::ostream& operator<<(std::ostream& out, const StoredDef& s)  // output
{
  if (s.within()) {
    out << "within ";
    if (s.name()) out << s.name().get();
    out << ";\n";
  }
  foreach_(ClassType e, s.classes()) out << e << ";" << std::endl;
  return out;
}
...
std::ostream& operator<<(std::ostream& out, const DefClass& c)
{
  ...
  if (c.encapsulated()) out << "encapsulated ";
  out << c.name() << " = ";
  foreach_(Option<TypePrefix> tp, c.type_prefixes()) out << typePrefix(tp);
  out << c.definition();
  if (c.modification()) out << c.modification().get();
  return out;
}
std::ostream& operator<<(std::ostream& out, const ExtendsClass& c)
{
  ...
  if (c.encapsulated()) out << "encapsulated ";
  out << "extends " << c.name();
  if (c.modification()) out << c.modification();
  return out;
}
...
{
  if (is<Class>(c)) return get<Class>(c).name();
  if (is<DefClass>(c)) return get<DefClass>(c).name();
  if (is<DerClass>(c)) return get<DerClass>(c).name();
  if (is<EnumClass>(c)) return get<EnumClass>(c).name();
  if (is<ExtendsClass>(c)) return get<ExtendsClass>(c).name();
