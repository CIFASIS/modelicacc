...
Component::Component(TypePrefixes tp, Name ty, Option<ExpList> ind, DeclList decl)
    : prefixes_(tp), type_(ty), indices_(ind), declarations_(decl)
{
  redeclare_ = false;
  final_ = false;
  inner_ = false;
...
{
  if (c.redeclare()) out << "redeclare ";
  if (c.final()) out << "final ";
  if (c.inner()) out << "inner ";
  if (c.outer()) out << "outer ";
  if (c.replaceable()) out << "replaceable ";
  foreach_(Option<TypePrefix> tp, c.prefixes()) out << typePrefix(tp);
  out << c.type() << " ";
...
{
  out << d.name();
  if (d.modification()) out << d.modification().get();
  if (d.conditional()) out << " if " << d.conditional().get();
  out << d.comment();
  return out;
}
...
std::ostream& operator<<(std::ostream& out, const Extends& e)  // output
{
  ...
  if (e.annotation()) out << e.annotation().get();
  return out;
}
std::ostream& operator<<(std::ostream& out, const ElemClass& e)  // output
{
  if (e.redeclare()) out << "redeclare ";
  if (e.final()) out << "final ";
  if (e.inner()) out << "inner ";
  if (e.outer()) out << "outer ";
  if (e.replaceable()) out << "replaceable ";
