...
ClassModification::ClassModification(Argument a) { push_back(a); }
...
std::ostream& operator<<(std::ostream& out, const ElMod& e)  // output
{
  if (e.each()) out << "each ";
  out << e.name();
  if (e.modification()) out << e.modification().get();
  return out;
}
...
bool ModEq::operator==(const ModEq& other) const { return (other.exp() == exp()); }

bool ModAssign::operator==(const ModAssign& other) const { return (other.exp() == exp()); }

member_imp(Constrained, Name, name);
member_imp(Constrained, Option<ClassModification>, modification);
bool Constrained::operator==(const Constrained& other) const { return (other.name() == name() && other.modification() == modification()); }

bool Component1::operator==(const Component1& other) const { return (other.type() == type() && other.declaration() == declaration()); }
...
ShortClass::ShortClass(ClassPrefixes cp, Name n, TypePrefixes tp, Name der, Option<ExpList> ind, Option<ClassModification> mod,
                       Comment comm)
    : prefixes_(cp), name_(n), derived_(der), indices_(ind), modification_(mod), comment_(comm)
{
  foreach_(Option<TypePrefix> t, tp)
  {
    if (t) {
      if (!type_prefixes_) type_prefixes_ = TypePrefixes();
...
{
  return (other.name() == name() && other.indices() == indices() && other.modification() == modification() &&
          other.conditional() == conditional());
}

bool ModClass::operator==(const ModClass& other) const { return (other.modification() == modification() && other.exp() == exp()); }
Declaration::Declaration() : indices_(Option<ExpList>()), conditional_(OptExp()){};
Declaration::Declaration(Name name) : name_(name){};
Declaration::Declaration(Name name, Option<ExpList> indices, Option<Modification> mod, OptExp cond)
    : name_(name), indices_(indices), modification_(mod), conditional_(cond){};
...
std::ostream& operator<<(std::ostream& out, const Comment& c)
{
  if (c.st_comment()) {
    ...
    int i = 0, l = st.size();
    out << " ";
    foreach_(const String& s, st) { out << s << (i++ < l - 1 ? " + " : ""); }
  }
  if (c.annotation()) {
    out << " ";
    SKIP_BLOCK_START;
