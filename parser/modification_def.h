...
BOOST_FUSION_ADAPT_STRUCT(Modelica::AST::ElMod, (Modelica::AST::Name, name_)(Option<Modelica::AST::Modification>,
                                                                             modification_)(Modelica::AST::StringComment, st_comment_))

BOOST_FUSION_ADAPT_STRUCT(Modelica::AST::Component1,
                          (Modelica::AST::TypePrefixes, prefixes_)(Modelica::AST::Name, type_)(Modelica::AST::Declaration, declaration_))

BOOST_FUSION_ADAPT_STRUCT(Modelica::AST::Declaration,
                          (Modelica::AST::Name, name_)(Option<Modelica::AST::ExpList>, indices_)(Option<Modelica::AST::Modification>,
                                                                                                 modification_))

BOOST_FUSION_ADAPT_STRUCT(Modelica::AST::ModClass, (Modelica::AST::ClassModification, modification_)(Modelica::AST::OptExp, exp_))

BOOST_FUSION_ADAPT_STRUCT(Modelica::AST::Constrained, (Modelica::AST::Name, name_)(Option<Modelica::AST::ClassModification>, modification_))

BOOST_FUSION_ADAPT_STRUCT(Modelica::AST::Enum, (Modelica::AST::Name, name_)(Modelica::AST::Comment, comment_))

BOOST_FUSION_ADAPT_STRUCT(Modelica::AST::Comment,
                          (Modelica::AST::StringComment, st_comment_)(Option<Modelica::AST::Annotation>, annotation_))
      ...
      FINAL("final")
{
  ...
  modification = equal_mod | assign_mod | class_mod;

  equal_mod = EQUAL > expression;

  assign_mod = ASSIGN > expression;
  ...
  element_redeclaration = (REDECLARE >> matches[EACH] >> matches[FINAL] >>
                           ((short_class_definition | component_clause1) | element_replaceable))[_val = construct<ElRedecl>(_1, _2, _3)];

  element_modification = expression.name >> (-modification) >> string_comment;

  element_replaceable =
      (REPLACEABLE >> (short_class_definition | component_clause1) >> (-constraining_clause))[_val = construct<ElRepl>(_1, _2)];

  // TODO add the + string
  string_comment = -(expression.string_ % PLUS);

  type_prefix = (-(FLOW | STREAM)) >> (-(DISCRETE | PARAMETER | CONSTANT)) >> (-(INPUT | OUTPUT));

  component_clause1 = type_prefix >> expression.name >> component_declaration1;

  component_declaration1 = (declaration >> comment)[_val = bind(&add_comment<Declaration>, _1, _2)];

  declaration = expression.ident >> (-expression.array_subscripts) >> (-modification);

  constraining_clause = CONSTRAINEDBY > expression.name > (-class_modification);

  class_mod = class_modification >> -(EQUAL > expression);

  // TODO: return something meaningful
  short_class_definition =
      (class_prefixes >> expression.ident >> EQUAL >> type_prefix >> expression.name >> (-expression.array_subscripts) >>
       (-class_modification) >> comment)[_val = construct<ShortClass>(_1, _2, _3, _4, _5, _6, _7)] |
      (class_prefixes >> expression.ident >> EQUAL >> ENUMERATION >> OPAREN >> enum_spec >> CPAREN >>
       comment)[_val = construct<ShortClass>(_1, _2, _3, _4)];

  /* Error and debug */
  on_error<fail>(modification, std::cerr << val("Parser error. Expecting ") << _4  // what failed?
