...
struct ModelicaRealNumberPolicy : qi::strict_ureal_policies<T> {
  ...
  static bool parse_nan(Iter& first, Iter const& last, Attribute& attr)
  {
    return false;
  }

  template <typename Iter, typename Attribute>
  static bool parse_inf(Iter& first, Iter const& last, Attribute& attr)
  {
    return false;
  }
};
...
struct ExpressionRule : qi::grammar<Iterator, Skipper<Iterator>, Modelica::AST::Expression()> {
  ...
  qi::rule<Iterator, Skipper<Iterator>, ExpList()> function_call_args, function_arguments, named_arguments, expression_list,
      array_subscripts;
  ...
  /* Operators tokens */
  qi::rule<Iterator> OPAREN, CPAREN, COLON, SEMICOLON, QUOTE, OBRACKET, CBRACKET, COMA, OBRACE, CBRACE, EQUAL, ELSE, ELSEIF, IF, IN, FOR,
      THEN;
