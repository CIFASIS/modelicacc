...
equation
  ...
  // X axis row
  for i in 2:N loop
    der(u[i,1,1])=(-u[i,1,1]*ax/dx)+(-u[i,1,1]+u[i-1,1,1])*ay/dy + (-u[i,1,1]*az/dz) + reaction[i,1,1];
  end for;
  // Y axis row
  for j in 2:M loop
  ...
  // Z axis row
  for k in 2:O loop
    der(u[1,1,k])=(-u[1,1,k]*ax/dx)+(-u[1,1,k]+u[1,1,k-1])*ay/dy + (-u[1,1,k]*az/dz) + reaction[1,1,k];
  end for;
  // XY plane 
