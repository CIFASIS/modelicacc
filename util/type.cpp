...
bool Class::operator==(const Class& other) const { return (other.name() == name()); }
...
std::ostream& operator<<(std::ostream& out, const TypeDef& c)
{
  ...
  if (c.indices()) {
    ...
    foreach_(Expression e, el) out << "[" << e << "]";
  }
...
bool Array::operator==(const Array& other) const { return (other.dim() == dim() && arrayOf() == other.arrayOf()); }
...
bool Function::operator==(const Function& other) const { return (other.out() == out() && args() == other.args()); }
