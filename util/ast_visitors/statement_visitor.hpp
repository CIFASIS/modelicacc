...
#define applyExp(X) boost::apply_visitor(v, X)
  ...
  Statement operator()(Assign st) const
  {
    ...
    if (st.rl()) {
      foreach_(Expression & e, st.rl_ref().get()) rl.push_back(applyExp(e));
  ...
  Statement operator()(Break st) const { return st; };

  Statement operator()(Return st) const { return st; }
  ...
  Statement operator()(CallSt st) const
  {
    ...
    ExpList list;
    OptExpList out;
    foreach_(Expression s, st.arg()) list.push_back(applyExp(s));

    foreach_(OptExp s, st.out()) if (s) out.push_back(OptExp(applyExp(s.get())));
  ...
  Statement operator()(ForSt st) const
  {
    StatementList eqs;
    IndexList index;
    foreach_(Statement e, st.elements()) eqs.push_back(ApplyThis(e));
    foreach_(Index i, st.range().indexes()) if (i.exp()) index.push_back(Index(i.name(), OptExp(applyExp(i.exp().get()))));
  ...
  Statement operator()(IfSt st) const
  {
    ...
    foreach_(Statement e, st.elements()) elements.push_back(ApplyThis(e));

    foreach_(Statement e, st.ifnot()) elses.push_back(ApplyThis(e));

    foreach_(Else el, st.elseif())
    {
      StatementList list;
      foreach_(Statement s, get<1>(el)) list.push_back(ApplyThis(s));
  ...
  Statement operator()(WhenSt st) const
  {
    ...
    {
      StatementList list;
      foreach_(Statement e, get<1>(el)) list.push_back(ApplyThis(e));
