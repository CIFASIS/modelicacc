...
#include <util/ast_visitors/contains_vector.hpp>
#include <util/debug.hpp>
#include <boost/variant/get.hpp>
#include <causalize/vg_implementation/vector/vector_graph_definition.hpp>
...
ContainsVector::ContainsVector(Expression e, VectorVertexProperty v, const VarSymbolTable &s)
    : exp(e), unk2find(v), syms(s), foreq(false), indexes(){};
...
ContainsVector::ContainsVector(VectorVertexProperty unk, VarSymbolTable &s, IndexList indexes)
    : exp(unk.unknown()), unk2find(unk), syms(s), foreq(true), indexes(indexes)
{
  EvalExpression ev(syms);
  foreach_(Index i, indexes)
  {
    if (!i.exp()) ERROR("No index in for equation");
...
bool ContainsVector::operator()(Call call) const
{
  if (is<Call>(exp)) {  // exp is a derivative expression
    ...
    foreach_(Expression e, call.args()) { findOccur |= (ApplyThis(e)); }
    return findOccur;
  }
...
void ContainsVector::BuildPairs(Reference unkRef) const
{
  if (foreq) {                              // The equation is a for-equation
    if (unk2find.unknown.dimension == 0) {  // the unknown is a scalar
      ...
      foreach_(Expression val, indexes_list)
      {
        if (is<Modelica::AST::Integer>(val)) {
          ...
          if (!i.exp()) ERROR("Implicit range not supported in for equations");
          ...
          ERROR_UNLESS(binop.op() == Add || binop.op() == Sub, "Only addition and substraction are supported in indexes");
      ...
      ERROR_UNLESS(total_index_uses == 0 || total_index_uses == forIndexIntervalList.size(),
                   "The number of indexes does not match the number of uses");
      MDI mdi_eq(forIndexIntervalList), mdi_unk(unk_indexes);
      ERROR_UNLESS(mdi_eq.Size() == mdi_unk.Size() || mdi_eq.Size() == 1 || mdi_unk.Size() == 1, "Edge of different size");
    ...
    if (unk2find.unknown.dimension == 0) {  // the unknown is a scalar
      ...
      ERROR_UNLESS(unk2find.unknown.dimension == (int)get<1>(unkRef.ref().front()).size() || (int)get<1>(unkRef.ref().front()).size() == 0,
                   "Only complete usage of vectors are allowed");
      VarSymbolTable vst = syms;
      Expression unkRef_exp = unkRef;
      Expression evaluated_expr = Apply(Modelica::PartialEvalExpression(vst), unkRef_exp);
      IntervalList unk_indexes;
      ...
      if (indexes.size() == 0)  // If no usage, use the whole array
        for (int i : unk2find.unknown.dimensionList) unk_indexes.push_back(Interval::closed(1, i));
