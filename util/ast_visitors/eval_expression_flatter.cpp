...
EvalExpFlatter::EvalExpFlatter(const VarSymbolTable &v, Name n, Real r) : vtable(v), name(n), val(r){};
...
Interval EvalExpFlatter::operator()(BinOp v) const
{
  ...
  if (ll.card() == 1 && rr.card() == 1) {
    ...
    switch (v.op()) {
    case Add: {
      Interval i(ll.lo() + rr.lo(), rr.step(), ll.hi() + rr.hi());
      return i;
    }
    case Sub: {
      Interval i(ll.lo() - rr.lo(), rr.step(), ll.hi() - rr.hi());
    ...
    case Mult: {
      Interval i(ll.lo() * rr.lo(), ll.lo() * rr.step(), ll.hi() * rr.hi());
      return i;
    }
    default: {
    ...
    switch (v.op()) {
    case Add: {
      Interval i(ll.lo() + rr.lo(), ll.step(), ll.hi() + rr.hi());
      return i;
    }
    case Sub: {
      Interval i(ll.lo() - rr.lo(), ll.step(), ll.hi() - rr.hi());
...
Interval EvalExpFlatter::operator()(Range v) const
{
  ...
  if (newLo <= newHi) return Interval(newLo, newStep, newHi);
...
Interval EvalExpFlatter::operator()(Reference v) const
{
  ...
  if (is<ModClass>(m)) {
    ...
    if (oe) return Apply(evexp, oe.get());
