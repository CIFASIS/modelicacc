...
void ConvertToMicro::convert()
{
  ...
  foreach_(Equation & eq, eqs)
  {
    if (is<Equality>(eq)) {
      ...
      if (get<CallEq>(eq).name() == "assert") remove.push_back(eq);
    } else if (is<ForEq>(eq)) {
      ...
      if (oe) {
        ...
        if (is<Range>(exp)) {
          ...
          if (is<Integer>(range.end()) && is<Integer>(range.start()) &&
              (get<Integer>(range.end()) < get<Integer>(range.start()))) {  // Removing empty for eq
            remove.push_back(eq);
          }
        }
      }
  ...
  foreach_(Equation eq, remove) { eqs.erase(std::find(eqs.begin(), eqs.end(), eq)); }
  ...
  foreach_(Equation eq, remove) { init_eqs.erase(std::find(init_eqs.begin(), init_eqs.end(), eq)); }
  ConvertToMicroStatement tom(mmo_class, disc_count);
  foreach_(Statement & st, mmo_class.statements_ref().statements_ref()) { st = Apply(tom, st); }
  // Add new statements
  foreach_(Statement st, tom.statements()) { mmo_class.statements_ref().statements_ref().push_back(st); }
  foreach_(Name var, mmo_class.variables())
  {
    ...
    if (vinfo.type() == "Boolean" || vinfo.type() == "Integer") {
      // if ((vinfo.type()=="Boolean" && !isParameter(var,mmo_class.syms_ref())) || (vinfo.type()=="Integer" &&
      // !isParameter(var,mmo_class.syms_ref()) && !isConstant(var,mmo_class.syms_ref())))
      ...
      if (vinfo.type() == "Boolean" && vinfo.modification()) {
        ...
        if (is<ModEq>(mod)) {
          ...
          if (mc.exp()) mc.exp_ref() = Apply(tomicro_exp, mc.exp_ref().get());
      ...
      if ((!isConstant(var, mmo_class.syms_ref()) && !isParameter(var, mmo_class.syms_ref())) || (vinfo.type() == "Boolean"))
        vinfo.type_ref() = "Real";
    }
    if (vinfo.modification()) {
      ...
      if (is<ModEq>(mod)) {
        ...
        if (is<Brace>(exp)) {
          ...
          StatementList sl;
          int index = 1;
          foreach_(Expression e, get<Brace>(exp).args()) { sl.push_back(Assign(Reference(Ref(1, RefTuple(var, ExpList(1, index++)))), e)); }
          mmo_class.initial_sts_ref().statements_ref() += sl;
          TypePrefixes tp;
          bool was_const = false;
          foreach_(Option<TypePrefix> otp, vinfo.prefixes())
          {  // Remove constant
            if (otp && (otp.get() == constant || otp.get() == parameter)) {
              was_const = true;
            }
            if (otp && otp.get() != constant && otp.get() != parameter) tp.push_back(otp);
          }
          if (was_const) tp.push_back(parameter);
          ...
          if (ref.ref().size() == 1) {
            ...
            if (exp_l.size() == 1) {
              ...
              if (is<Range>(ind)) {
                ForSt fs(Indexes(IndexList(1, Index("i", ind))),
                         StatementList(1, Assign(Reference(Ref(1, RefTuple(var, ExpList(1, Reference("i"))))),
                                                 OptExp(Reference(Ref(1, RefTuple(n, ExpList(1, Reference("i")))))))));
                ModClass mc;
                ClassModification cm;
                ElMod em("fixed", ModEq(Boolean(FALSE)));
                em.each_ref() = true;
...
Equation ConvertToMicro::operator()(CallEq c) { return c; }
Equation ConvertToMicro::operator()(WhenEq c)
{
  ...
  if (is<Brace>(cond)) {
    ...
    c.cond_ref() = cond;
  if (c.elsewhen().size()) ERROR("Elsewhen not supported yet");
...
Equation ConvertToMicro::operator()(ForEq c) { return c; }
Equation ConvertToMicro::operator()(IfEq c) { return c; }
}  // namespace Modelica
