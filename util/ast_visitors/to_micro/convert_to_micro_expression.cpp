...
ConvertToMicroExpression::ConvertToMicroExpression(MMO_Class &cl, unsigned int &discont, bool w, bool in_alg)
    : mmo_class(cl), disc_count(discont), when(w), in_algorithm(in_alg){};
Expression ConvertToMicroExpression::operator()(Integer v) const { return v; }
...
Expression ConvertToMicroExpression::operator()(Name v) const { return v; }
Expression ConvertToMicroExpression::operator()(Real v) const { return v; }
Expression ConvertToMicroExpression::operator()(SubEnd v) const { return v; }
Expression ConvertToMicroExpression::operator()(SubAll v) const { return v; }
Expression ConvertToMicroExpression::operator()(BinOp v) const
{
  if (in_algorithm) return v;
  if (isRelation(v)) {
...
Expression ConvertToMicroExpression::operator()(Range v) const { return v; }
Expression ConvertToMicroExpression::operator()(Brace v) const { return v; }
Expression ConvertToMicroExpression::operator()(Bracket v) const { return v; }
Expression ConvertToMicroExpression::operator()(Call v) const
{
  ...
  if ("abs" == v.name()) {
    ...
    if (!when) ERROR("Call to sample in dynamic equation");
...
Expression ConvertToMicroExpression::operator()(FunctionExp v) const { return v; }
Expression ConvertToMicroExpression::operator()(ForExp v) const { return v; }
Expression ConvertToMicroExpression::operator()(Named v) const { return v; }
Expression ConvertToMicroExpression::operator()(Output v) const
{
  foreach_(OptExp & oe, v.args_ref()) if (oe) oe = ApplyThis(oe.get());
  if (v.args().size() == 1 && v.args().front() && is<Reference>(v.args().front().get())) return v.args().front().get();
  return v;
}
Expression ConvertToMicroExpression::operator()(Reference v) const { return v; }
