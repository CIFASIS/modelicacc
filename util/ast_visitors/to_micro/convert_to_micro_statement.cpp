...
Statement ConvertToMicroStatement::operator()(Assign v) const
{
  ...
  l = Apply(tom, l);
  if (opt_r) opt_r = Apply(tom, opt_r.get());
  v.left_ref() = l;
  v.right_ref() = opt_r;
  return v;
}

Statement ConvertToMicroStatement::operator()(Break v) const { return v; }

Statement ConvertToMicroStatement::operator()(Return v) const { return v; }
...
Statement ConvertToMicroStatement::operator()(IfSt v) const
{
  ...
  foreach_(Statement & s, v.ifnot_ref()) s = ApplyThis(s);
  foreach_(IfSt::Else & es, v.elseif_ref())
  {
    foreach_(Statement & s, get<1>(es)) s = ApplyThis(s);
