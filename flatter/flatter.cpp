    ...
    if (m) {
      ...
      if (is<Type::Class>(t_final)) {
        ...
        if (down.buffer()) { // Add external Object 
          ...
          //std::cout << "Adding " << n << " to " << c.name() << " of type " << v.type() << std::endl;
          //vsd.insert(n, VarInfo());
          buffers.insert(v.type());
        }
      }
    }
  }
...
void Flatter::Flat(MMO_Class &c, bool flatConnector, bool initial) {
  ...
  MarkConnector mc;
  EquationList couples, &eqs= c.equations_ref().equations_ref();
  foreach_(Equation &eq, eqs) {
    eq = Apply(mc, eq);
    if (is<CallEq>(eq) && "couple"==get<CallEq>(eq).name()) {
      couples.push_back(eq);
    }
  }
  StatementList sts;
  for (Equation &e : couples){  // Remove couple equations and add them as 
                                      // initial statements
    eqs.erase(std::find(eqs.begin(), eqs.end(), e));
    CallEq call = get<CallEq>(e);
    sts.push_back(CallSt(call.name(), call.args()));
  }
  if (sts.size()) 
    c.addInitStatement(WhenSt(Call("initial", ExpList()),sts));
