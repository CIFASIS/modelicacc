...
void Remove_Composition::LevelUp(MMO_Class &up, MMO_Class &down, Name nUp , VarInfo viUp)
{
				...
				if (is<ModEq>(mod)) {
					...
					foreach_(Argument &arg, mc.modification_ref()) {
						if (is<ElMod>(arg)) {
							...
							if ("start"==elm.name() || "fixed"==elm.name()) 
								elm.each_ref()=true;
						}
					}
				}
	...
	if (!viUp.indices()) {
		...
		if (eqsI.size() > 0)	
			up.addInitEquation(createForEquation(indexString,index , eqsI));
		if (sts.size() > 0)
			up.addStatement(createForStatement(indexString,index , sts));
		if (stsI.size() > 0)
			up.addInitStatement(createForStatement(indexString,index , stsI));	
