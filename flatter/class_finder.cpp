...
{
  foreach_(Equation eq, down.equations_ref().equations()) up.addEquation(eq);
  foreach_(Equation eq, down.initial_eqs().equations()) up.addInitEquation(eq);
  foreach_(Statement st, down.statements().statements()) up.addStatement(st);
  foreach_(Statement st, down.initial_sts().statements()) up.addInitStatement(st);
  ...
  foreach_(Extends i, down.extends_ref()) up.extends_ref().push_back(i);
}
  ...
  foreach_(Extends e, exls)
  {
    ...
    if (oc) {
      ...
      if (is<Type::Class>(t_final)) {
        ...
        ExpandAll(d);
        if (e.modification()) applyClassModification(up, d, e.modification().get());
        expand(up, d);
...
Option<typeContexTuple> ClassFinder::findTypeByName(MMO_Class &c, Name n)
{
  ...
  newMMO(contex, c) if (t) { return typeContexTuple(t.get(), *contex); }
  else if (c.father())
  {
...
{
  ExpandAll(target);
  foreach_(Argument a, m) applyArgument(contex, target, a);
}
  ...
  if (is<ElMod>(m)) {
    ...
    if (opv) {
      ...
      if (v.modification()) {
        ...
        v.set_modification(mod.modification());  // Caso: No habia modificaciones
  ...
  if (is<ShortClass>(aux)) {  // Redeclare of types
    ...
    if (shClass.enum_spec()) {  // Cambio de enum
      ...
      if (otd) {
        ...
        Type::TypeDef tt =
            Type::TypeDef(shClass.prefixes(), shClass.name(), preType, shClass.derived().get(), optIndex, shClass.modification());
        // target.tyTable_ref().erase(shClass.name());
        target.tyTable_ref().insert(shClass.name(), tt);
      } else
