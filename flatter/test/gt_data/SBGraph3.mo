...
equation
  for i in 1:1:1,j in 1:1:1 loop
    G_p_eff[i,j] = S_n_eff[i,j];
  end for;
  for i in 1000:1:1000,j in 1:1:1000 loop
    C_d_eff[i,j] = S_n_eff[i-999,1];
  end for;
  for i in 1:1:1,j in 1:1:1 loop
    S_n_fl[i,j]+sum(C_d_fl[i+999, 1:1:1000])+G_p_fl[i,j] = 0;
  end for;
  ...
  for i in 1:1:1,j in 1:1:1000 loop
    C_u_eff[i,j] = S_p_eff[i,1];
  end for;
  for i in 1:1:1,j in 1:1:1 loop
    sum(C_u_fl[i, 1:1:1000])+S_p_fl[i,j] = 0;
  end for;
