...
class GraphPrinter {
  ...
  typedef typename boost::adjacency_list<boost::listS, boost::listS, boost::undirectedS, VertexProperty, EdgeProperty>::vertex_descriptor
      Vertex;
  typedef typename boost::adjacency_list<boost::listS, boost::listS, boost::undirectedS, VertexProperty, EdgeProperty>::out_edge_iterator
      EdgeIterator;
  ...
  void printGraph(std::string name)
  {
    ...
    for (Iterator it = equationDescriptors.begin(); it != equationDescriptors.end(); it++) {
      ...
      if (out_degree(*it, graph) == 0) colors2 << "    " << graph[*it].index << "[ color=\"red\" ];" << endl;
    }
    for (Iterator it = equationDescriptors.begin(); it != equationDescriptors.end(); it++) {
      MAKE_SPACE
#ifdef HAS_COUNT
