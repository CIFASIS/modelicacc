...
VectorCausalizationGraph ReducedGraphBuilder::makeGraph()
{
  ...
  if (debugIsEnabled('c')) {
    DEBUG_MSG("Equations");
    foreach_(VectorEquationVertex eq, equationDescriptorList) { DEBUG_MSG(graph[eq].index << ": " << graph[eq].equation); }
    DEBUG_MSG("Unknowns");
    foreach_(VectorUnknownVertex un, unknownDescriptorList) { DEBUG_MSG(graph[un].index << ": " << graph[un].unknown()); }
  }
...
int ReducedGraphBuilder::getForRangeSize(ForEq feq)
{
  ...
  ERROR_UNLESS((bool)i.exp(), "graph_builder:\n No expression on for equation");
  Expression exp = i.exp().get();
  if (is<Brace>(exp)) return get<Brace>(exp).args().size();
  if (is<Range>(exp)) {
    Range range = get<Range>(exp);
